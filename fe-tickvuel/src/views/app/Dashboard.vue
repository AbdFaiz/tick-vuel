...
const handleDeleteTicket = async (code) => {
...
<template>
    ...
    <div class="space-y-4">
        <!-- Ticket Card 1 -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow relative"
            v-for="ticket in tickets" :key="ticket.code">
            <button @click.stop.prevent="handleDeleteTicket(ticket.code)" v-if="user?.id === ticket.user_id || user?.role === 'admin'"
                class="absolute top-4 right-12 p-2 text-gray-400 hover:text-red-600 transition-colors z-10">
